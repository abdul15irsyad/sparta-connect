...
@section('content')
    <div class="content-wrapper roles-edit">
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                @endif
                <form action="{{ route('admin.roles.update', ['id' => $role->id]) }}" method="post" autocomplete="off">
                    @csrf
                    @method('put')
                    <div class="card">
                            ...
                            <div class="form-group col-12">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                        name="name" placeholder="eg: Administrator" value="{{ old('name', $role->name) }}"
                                        {{ in_array($role->slug, ['super-admin', 'admin']) ? 'disabled' : '' }}>
                                    @error('name')
                            ...
                            <div class="form-group col-12">
                                ...
                                <div class="input-group">
                                    <textarea class="form-control @error('desc') is-invalid @enderror" id="desc" name="desc"
                                        ...
                                        rows="4">{{ old('desc', $role->desc) }}</textarea>
                                    @error('desc')
                                        <span class="invalid-feedback pl-2" role="alert">{{ $message }}</span>
                                    @enderror
                                </div>
                                    ...
                                    <select class="form-control select2 @error('permissions') is-invalid @enderror"
                                        ...
                                        @foreach ($permissions as $permission)
                                            <option value="{{ $permission->slug }}"
                                                <?= in_array($permission->slug, old('permission', $role_permissions) ?? []) ? 'selected' : '' ?>>
                        ...
                        <div class="card-footer">
                            <div class="row justify-content-end">
                                <div class="col-auto">
                                    <a href="{{ url()->previous() != route('admin.roles.index')? route('admin.roles.show', ['id' => $role->id]): route('admin.roles.index') }}"
